...
import ideatc.jinxiang.fragment.matter_detail_acitivity.BasicInfoFragment;
import ideatc.jinxiang.fragment.matter_detail_acitivity.HandleOpinionFragment;
import ideatc.jinxiang.fragment.matter_detail_acitivity.SysFlowLogFragment;
import ideatc.jinxiang.utils.ParamsUtil;
import ideatc.jinxiang.utils.SPUtils;

public class MatterDetailActivity extends ToolBarBaseActivity {
...
//    private ArrayList<SysFlowInfoBean.SysFlowLogEntity> logInfoList = new ArrayList<>();
    ...
    private void getSysFlowInfo() {
        ...
        Log.e("单据参数", "sysUserId=" + SPUtils.getInt("id", 0) + ",sysFlowId=" + rowsEntity.getSysFlowId() + ",keyId=" + rowsEntity.getKeyId());
        httpClient().get(url, params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                Log.e("流程信息", getString(responseBody));
//                SysFlowInfoBean sysFlowInfoBean = new Gson().fromJson(getString(responseBody), SysFlowInfoBean.class);
                ...
                try {
                    ...
                    String buttonStr = object.getJSONArray("Buttons").toString();
                    String modelName = object.getString("ModelName");
                    fragments.add(BasicInfoFragment.newInstance(modelName, orderStr, buttonStr, rowsEntity));
                    fragments.add(HandleOpinionFragment.instance(logStr));
                    fragments.add(SysFlowLogFragment.instance(logStr));
